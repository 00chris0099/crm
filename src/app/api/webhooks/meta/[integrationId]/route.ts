...
export async function GET(req: Request, { params }: { params: { integrationId: string } }) {
    // Verificación de webhook con Meta usando custom config del integration_id
    const { searchParams } = new URL(req.url);
    const mode = searchParams.get('hub.mode');
    const token = searchParams.get('hub.verify_token');
    const challenge = searchParams.get('hub.challenge');

    // Lógica futura: buscar la integración (params.integrationId), sacar su verify_token esperado.
...
export async function POST(req: Request, { params }: { params: { integrationId: string } }) {
    ...
    try {
        ...
        return NextResponse.json({ success: true, integration: params.integrationId }, { status: 200 });
    } catch(err) {
